...
function binarySearchForMinNumberValidSplits(message: string, limit: number): number {
    let minSplits = 0;
...
function calculateNumberOfRemainingInputCharsAfterLastSplits(totalInputChars: number, limit: number, totalSplits: number, numberOfDigitsInTotalSplitParts: number): number {
...
function createSplitMessage(message: string, limit: number, totalSplits: number): string[] {
    let indexResult = 0;
    let indexStartSplit = 0;
    let numberOfSplitMessages = totalSplits + 1;
    const resultSplitMessage: string[] = new Array(numberOfSplitMessages);
