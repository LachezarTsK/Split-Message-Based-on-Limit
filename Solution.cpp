...
class Solution {
    ...
    struct NOT_POSSIBLE {
        ...
        inline static const vector<string> AS_STRING_VECTOR{};
        static const int AS_NEGATIVE_INTEGER = -1;
        static const int AS_POSITIVE_INTEGER = numeric_limits<int>::max();
    };

    /*
    Suffix scaffolding is </>
     */
    static const int SUFFIX_SCAFFOLDING_SIZE = 3;
     ...
     */
    static const int MIN_NUMBER_OF_DIGITS_IN_SUFFIX = 2;
    static const int MIN_SUFFIX_SIZE = SUFFIX_SCAFFOLDING_SIZE + MIN_NUMBER_OF_DIGITS_IN_SUFFIX;

public:
    vector<string> splitMessage(string message, int limit) const {
        if (limit <= MIN_SUFFIX_SIZE) {
            return NOT_POSSIBLE::AS_STRING_VECTOR;
        }
...
private:
    int binarySearchForMinNumberValidSplits(const string& message, int limit) const {
        int minSplits = 0;
        ...
        while (minSplits <= maxSplits) {
            ...
            int numberOfDigitsInTotalSplitParts = floor(log10(numberOfSplits + 1)) + 1;
    ...
    int calculateNumberOfRemainingInputCharsAfterLastSplits(int totalInputChars, int limit, int totalSplits, int numberOfDigitsInTotalSplitParts) const {
        ...
        while (remainingSplits > 0) {
            ...
            int currentNumberOfSplits = min(upperLimitNumberOfDigits * 10 - upperLimitNumberOfDigits, remainingSplits);
    ...
    vector<string> createSplitMessage(const string& message, int limit, int totalSplits) const {
        int indexResult = 0;
        int indexStartSplit = 0;
        int numberOfSplitMessages = totalSplits + 1;
        vector<string> resultSplitMessage(numberOfSplitMessages);

        for (int messageNumber = 1; messageNumber <= numberOfSplitMessages; ++messageNumber) {
            string suffix = "<" + to_string(messageNumber) + "/" + to_string(numberOfSplitMessages) + ">";
            int indexEndSplit = min(indexStartSplit + (limit - suffix.length() - 1), message.length() - 1);
            resultSplitMessage[indexResult] = message.substr(indexStartSplit, indexEndSplit - indexStartSplit + 1) + suffix;
